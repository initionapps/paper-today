  ...
  return (
    <header>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-[2.1rem] font-bold leading-tight tracking-[-0.022em] text-text">
            {weekdayName(day)}
          </h1>
          <p className="mt-1 text-[13.5px] text-text-2">{longDate(day)}</p>
        </div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        {hasWindows && (
          <span className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px] text-text-3">
        ...
        <WorkHoursEditor day={day} className="ms-auto" />
      </div>
    </header>
  );
}
...
function NavButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg text-text-3 transition-colors hover:bg-surface hover:text-text-2"
    >
      {children}
    </button>
  );
